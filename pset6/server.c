...
{
    // TODO
    char *pathIndexphp = (char*)malloc(strlen(path) + strlen("index.php") + 1);
    char *pathIndexHTML = (char*)malloc(strlen(path) + strlen("index.html") + 1);
    strcpy(pathIndexphp, path);
    strcpy(pathIndexHTML, path);
    strcat(pathIndexphp, "index.php");
    strcat(pathIndexHTML, "index.html");
    
    //find path for php file.
    if (access(pathIndexphp, F_OK) == 0){
        free(pathIndexHTML);
        return pathIndexphp;
    }
    
    //find path for html file if php file not found
    else if (access(pathIndexHTML, F_OK) == 0 ){
        free(pathIndexphp);
        return pathIndexHTML;
    }
    
    //free memory on heap if both php and html files are not found.
    free(pathIndexphp);
    free(pathIndexHTML);
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    //find file size and fread all file immediately. this might not workout.
    // an option is provided.
    int file_read;
    fseek(file, 0, SEEK_END);
    file_read = ftell(file);
    fseek(file, 0, SEEK_SET);
    
    // checks if it was able to get file size.
    if (file_read  > -1){
        *content = malloc(file_read);
        fread(*content, 1, file_read, file);
        *length = file_read;
        return true;
    }
    //else read file waiting to see the end
    else{
        BYTE *burfer = malloc(512);
        while(true){
            file_read = fread(burfer, 1, 512, file);
            if (file_read < 512){
                *length += file_read;
                *content = burfer;
                return true;
            } 
            if ((burfer = realloc(burfer, *length + 512)) == NULL){
                return false;
            }
            *length += file_read;
        }
...
const char* lookup(const char* path)
{
    ...
    char *string = strchr(path, '.');
    
    if (strcasecmp(string, ".css") == 0)
    {
        return "text/css";
    }
    ...
    else if (strcasecmp(string, ".HTML") == 0)
    {
        return "text/html";
    }
...
{
    // create a copy of line that can be tossed comfortably.
    char copyLine[strlen(line)];
    strcpy(copyLine, line);
    copyLine[strlen(line)] = '\0';
    
    //find all three space in line.
    ...
    int i, j, k = 0;
    //ensure i got the correct method.
    char method1[10];
    for (i = 0; line[i] != ' '; i++){
        method[i] = line[i];
    }
    ...
    //find double space.
    char * doubleSpace = strchr(line, '.');
    if (doubleSpace != NULL){
        for (int m = 0; m < strlen(doubleSpace - 5); m++){
            if ((doubleSpace[m] == ' ') && (doubleSpace[m+1] == ' ')){
                error(400);
                return false;
            }
        }
    ...
    //now find target line
    char abs_path_copy[LimitRequestLine + 1], query_copy[LimitRequestLine + 1], questionmark[1];
    int n = strlen(target);
    for (j = 0; j<n ; j++){
        if (target[j] == '?'){
            questionmark[0] = target[j];
            break;
        }
        if (target[j] =='\"'){
            error(400);
        ...
        abs_path_copy[j] = target[j];
    }
    
    abs_path_copy[j] = '\0';
    if (target[j] == '?'){
        j++;
        n = n - (strlen(abs_path_copy));
        for (k = 0; k<n; k++){
            if (target[j] == '\"'){
                error(400);
                return false;
            }
            query_copy[k] = target[j];
            j++;
        }
    }
    query_copy[k] = '\0';
